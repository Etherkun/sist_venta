...
@section ('contenido')
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			...
			{!! Form::model($cliente, ['method'=>'PATCH','route'=>['sistema.cliente.update',$cliente->idcliente] ]) !!}
			{{Form::token()}}
			<div class="form-group">
				<label for="nombre">Nombre(s) y Apellido(s)</label>
				<input type="text" name="nombre" onkeypress="return soloLetras(event)" class="form-control" value="{{ $cliente->nombre }}" placeholder="Nombre(s) y Apellido(s) ...">
			</div>

			<div class="form-group">
				<label for="nombre">Cédula / RIF</label>
				<input type="text" name="cedula_rif" class="form-control" value="{{ $cliente->cedula_rif }}" placeholder="Cédula / RIF ...">
			</div>

			<div class="form-group">
				<label for="nombre">Teléfono</label>
				<input type="text" name="telefono" class="form-control" value="{{ $cliente->telefono }}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" placeholder="Télefono ...">
			</div>
